  ...
  for filterNum = 1:numFilters
    ...
    for i = 1:convDim/poolDim
        for j =1:convDim/poolDim
            poolregion=resp((i-1)*poolDim+1:i*poolDim,(j-1)*poolDim+1:j*poolDim);
            act(i,j)=sqrt(sum(sum(poolregion.^2))+params.epsilon);
        end
    end
    %poolConvFeatures = conv2(convolvedFeatures(:,:,filterNum,imageNum),poolFilter,'valid');
    %pooledFeatures(:,:,filterNum,imageNum) = poolConvFeatures(1:poolDim:end,1:poolDim:end);
